...
function getNodeColor(node) {
  ...
  // Priority 1: CIB flagged nodes (always red)
  if (node.suspicious) {
    return '#b91c1c';
  }

  // Priority 2: Community colors (if communities detected)
  if (communities?.communities) {
    const cid = communities.communities.get(node.id);
    const noiseId = communities.noiseClusterId;

    // Noise cluster (gray)
    if (cid === noiseId || cid === 0) {
      return NODE_TYPE_COLORS.noise;
    }

    // Valid community - apply community color
    if (cid !== undefined && cid > 0) {
      return communityColors[(cid - 1) % communityColors.length];
    }
  }

  // Priority 3: Fallback to type-based colors (when no communities detected)
  if (node.type === 'user') {
    ...
    usertags.forEach(tag => {
      ...
      if (!nodeMap.has(taggedId)) {
        nodeMap.set(taggedId, {
          ...
          followers: taggedUser.follower_count || 0
        });
      }
...
function displayMetrics(metrics) {
  ...
  // Add modularity if communities are detected
  if (communities && communities.modularity !== undefined) {
    const modularityValue = communities.modularity.toFixed(3);
    const modularityClass = communities.modularity > 0.3 ? 'good-modularity' : '';
    html += `<li><span class="metric-name">Modularity</span><span class="metric-value ${modularityClass}">${modularityValue}</span></li>`;
  }
...
function runLouvainCommunityDetection(graph) {
  ...
  if (baseState.totalWeight === 0) {
    ...
    return finalizeLouvainClusters(assignments, baseState.adjacency, baseState.nodeIds.length, graph);
  }
  ...
  return finalizeLouvainClusters(flattenedAssignments, baseState.adjacency, baseState.nodeIds.length, graph);
}
...
function finalizeLouvainClusters(assignments, adjacency, nodeCount, graph) {
  ...
  console.log(`Community filtering: min cluster size = ${minSize} for ${nodeCount} nodes`);
  ...
  clusterMembers.forEach((members, clusterId) => {
    ...
    const tooSmall = members.length < minSize;
    const isSingleton = members.length <= 1;

    if (isSingleton || tooSmall || !hasInternalEdges) {
      members.forEach(nodeId => noiseNodes.add(nodeId));
      filteredClusters.push({
        id: clusterId,
        size: members.length,
        reason: isSingleton ? 'singleton' : (tooSmall ? 'too small' : 'no internal edges')
      });
      return;
    }
  ...
  if (filteredClusters.length > 0) {
    console.log(`Filtered ${filteredClusters.length} clusters as noise:`);
    const summary = {};
    filteredClusters.forEach(c => {
      summary[c.reason] = (summary[c.reason] || 0) + 1;
    });
  ...
  if (graph && graph.links && graph.links.length > 0) {
    modularity = calculateModularity(graph, finalAssignments);
  }

  // Log community sizes
  const communitySizes = new Map();
  finalAssignments.forEach((cid, nodeId) => {
    communitySizes.set(cid, (communitySizes.get(cid) || 0) + 1);
  });
  const sortedSizes = Array.from(communitySizes.entries())
    .filter(([cid]) => cid !== 0)
    .sort((a, b) => b[1] - a[1])
    .map(([cid, size]) => `C${cid}:${size}`);

  console.log(`✓ Community detection complete: ${nonNoiseCount} communities (${sortedSizes.join(', ')}), ${noiseNodes.size} noise nodes, modularity: ${modularity.toFixed(3)}`);
...
function computeAdaptiveClusterThreshold(nodeCount) {
  // More lenient thresholds to preserve smaller legitimate communities
  // Changed from requiring 2-12+ nodes to more reasonable 2-6+ nodes
  if (nodeCount <= 50) return 2;    // Small networks: keep pairs
  if (nodeCount <= 150) return 3;   // Medium-small: keep triads
  if (nodeCount <= 400) return 3;   // Medium: still keep triads
  if (nodeCount <= 800) return 4;   // Medium-large: need 4+
  if (nodeCount <= 1500) return 5;  // Large: need 5+
  return Math.max(6, Math.floor(nodeCount * 0.005)); // Very large: 0.5% of network (max 6-15)
}
...
// Calculate modularity score for community structure quality
function calculateModularity(graph, communityAssignments) {
  if (!graph || !communityAssignments || graph.links.length === 0) {
    return 0;
  }
  ...
  graph.links.forEach(link => {
    ...
    degrees.set(sourceId, (degrees.get(sourceId) || 0) + weight);
    degrees.set(targetId, (degrees.get(targetId) || 0) + weight);
    totalWeight += weight;
  });

  if (totalWeight === 0) return 0;

  // Calculate modularity: Q = (1/2m) * sum_ij [A_ij - (k_i * k_j)/2m] * delta(c_i, c_j)
  let modularity = 0;
  ...
  graph.links.forEach(link => {
    ...
    const sourceCommunity = communityAssignments.get(sourceId);
    const targetCommunity = communityAssignments.get(targetId);

    // Only count edges within same community
    if (sourceCommunity !== undefined && sourceCommunity === targetCommunity) {
      ...
      const expectedEdges = (ki * kj) / (2 * totalWeight);
      modularity += weight - expectedEdges;
    }
  });

  // Normalize by 2m
  modularity = modularity / (2 * totalWeight);
...
function moveTooltip(event) {
  ...
  // Get the canvas container's position to convert viewport coords to container coords
...
detectBtn.addEventListener('click', () => {
  ...
  setTimeout(() => {
    ...
    // Calculate and display network metrics (including modularity)
    if (!networkMetrics) {
      networkMetrics = calculateNetworkMetrics(graphData);
    }
    displayMetrics(networkMetrics);
